...
namespace MarchingSquares
{
    ...
    public class VoxelGrid : MonoBehaviour
    {
        ...
        private void Awake()
        {
            boundsTrigger = GetComponent<BoxCollider2D>();
        }
        ...
        private void Update()
        {
            ...
            surface.EnableCollision(enteredColliders.Count > 0);
        }
        ...
        public void Initialize(VoxelMap map, int resolution, int chunkResolution, Vector2Int chunkPos, float size, float maxFeatureAngle)
        {
            ...
            if (map.overrideMaterial) surface.GetComponent<Renderer>().sharedMaterial = map.overrideMaterial;
        ...
        {
            var localPos = new Vector3((x + 0.5f) * voxelSize, (y + 0.5f) * voxelSize, -0.01f);
            //GameObject o = Instantiate(voxelPrefab) as GameObject;
            //o.transform.SetParent(transform);
            //o.transform.localPosition = localPos;
            //o.transform.localScale = Vector3.one * voxelSize * 0.05f;
            //o.SetActive(false);
            //voxelMaterials[i] = o.GetComponent<MeshRenderer>().material;
            voxels[i] = new Voxel(x, y, voxelSize, this.transform, localPos);

            SetMapBounds(i, voxels[i]);
        }

        private void SetMapBounds(int index, Voxel trans)
        {
            if (IsBottomLeft(index))
            {
                map.SetBottomLeftTrans(trans);
            }
            else if (IsTopRight(index))
            {
                map.SetTopRightTrans(trans);
            }
        }

        private bool IsTopRight(int index)
        {
            return index == resolution * resolution - 1
                      && chunkPos.x == chunkResolution - 1
                      && chunkPos.y == chunkResolution - 1;
        }

        private bool IsBottomLeft(int index)
        {
            return index == 0 && chunkPos.x == 0 && chunkPos.y == 0;
        }
