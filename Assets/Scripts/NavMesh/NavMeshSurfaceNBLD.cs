...
{
    public enum CollectObjectsNBLD
    {
        All = 0,
        Volume = 1,
        Children = 2,
    ...
    [AddComponentMenu("Navigation/NavMeshSurfaceNBLD", 30)]
    [HelpURL("https://github.com/Unity-Technologies/NavMeshComponents#documentation-draft")]
    public class NavMeshSurfaceNBLD : MonoBehaviour
    {
        [SerializeField]
        public bool m_CollectBoth = true;
        [SerializeField]
        int m_AgentTypeID;
        public int agentTypeID { get { return m_AgentTypeID; } set { m_AgentTypeID = value; } }

        [SerializeField]
        CollectObjectsNBLD m_CollectObjects = CollectObjectsNBLD.All;
        public CollectObjectsNBLD collectObjects { get { return m_CollectObjects; } set { m_CollectObjects = value; } }
        ...
        static readonly List<NavMeshSurfaceNBLD> s_NavMeshSurfaces = new List<NavMeshSurfaceNBLD>();

        public static List<NavMeshSurfaceNBLD> activeSurfaces
        {
        ...
        public void BuildNavMesh()
        {
            ...
            if (m_CollectObjects != CollectObjectsNBLD.Volume)
            {
        ...
        static void Register(NavMeshSurfaceNBLD surface)
        {
#if UNITY_EDITOR
        ...
        static void Unregister(NavMeshSurfaceNBLD surface)
        {
        ...
        Bounds CalculateWorldBounds(List<NavMeshBuildSource> sources)
        {
            ...
            if (collectObjects != CollectObjectsNBLD.Children)
            {
        ...
        private static Bounds CalculateGridWorldBounds(Matrix4x4 worldToLocal)
        {
            ...
            var tilemaps = FindObjectsOfType<Collider>();
            if (tilemaps == null || tilemaps.Length < 1)
            {

                throw new NullReferenceException("Add at least one Transform");
            }
            foreach (var tilemap in tilemaps)
            {
                //Debug.Log($"From Local Bounds [{tilemap.name}]: {tilemap.localBounds}");
                var lbounds = tilemap.bounds;
        ...
        bool UnshareNavMeshAsset()
        {
            ...
            var prefab = UnityEditor.PrefabUtility.GetCorrespondingObjectFromSource(this) as NavMeshSurfaceNBLD;
